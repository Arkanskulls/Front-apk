...
const ActiveTrainingsAthlete = () => {
    ...
    return (
        <View style={styles().container}>
            <HeaderPage
                onBack={() => navigation.navigate('Perfil')} title={"Treinos ativos"}/>
            <View style={styles().main}>
                {trainings.length > 0 ?
                    <ScrollView style={{ width: '100%', marginBottom: 20, paddingLeft: 15, paddingRight: 15 }}>
                        {trainings.map((item, i) => (
                            <TrainingItem fromActive training={item} key={`training-${i}`} />
                        ))}
                        {loading &&
